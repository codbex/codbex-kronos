...
/**
 * The listener interface for receiving HDBSchemaCore events. The class that is interested in
 * processing a HDBSchemaCore event implements this interface, and the object created with that
 * class is registered with a component using the component's <code>addHDBSchemaCoreListener</code>
 * method. When the HDBSchemaCore event occurs, that object's appropriate method is invoked.
 *
 */
public class HDBSchemaDefinitionListener extends HDBSchemaBaseListener {
    ...
    public void exitHdbSchemaDefinition(HDBSchemaParser.HdbSchemaDefinitionContext ctx) {
        if (ctx.schemaNameProp() != null && ctx.schemaNameProp()
                                               .STRING() != null) {
            model.setSchemaName(handleStringLiteral(ctx.schemaNameProp()
                                                       .STRING()
                                                       .getText()));
        }
    }
