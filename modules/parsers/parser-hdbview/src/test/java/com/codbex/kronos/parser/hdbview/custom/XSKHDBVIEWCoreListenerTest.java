...
import com.codbex.kronos.parser.hdbview.custom.XSKHDBVIEWCoreListener;
...
public class XSKHDBVIEWCoreListenerTest {
  ...
  public void parseHdbviewFileWithSingleQuote() throws Exception {
    ...
    assertEquals( osLineSeparator
        ...
        + "    from \"acme.db.status::t_status_model_transition\"" + osLineSeparator
        + "    union all" + osLineSeparator
        + "    select next_status_code as code, status_model_code" + osLineSeparator
        + "    from \"acme.db.status::t_status_model_transition\"" + osLineSeparator
        + ")" + osLineSeparator
        + "where status_model_code = 'acme.config.EVALUATION'" + osLineSeparator, model.getQuery());
  }
}
