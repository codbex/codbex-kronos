...
public class HDBSequenceDefinitionVisitor extends HDBSequenceBaseVisitor<JsonElement> {
    ...
    public JsonElement visitHdbSequenceDefinition(HdbSequenceDefinitionContext ctx) {
        ...
        for (ParseTree tree : ctxList) {
            if (tree.getChild(0) instanceof SchemaContext) {
                ...
                parsedObj.add(HDBSequenceConstants.INCREMENT_BY_PROPERTY, visitIncrementBy((IncrementByContext) tree.getChild(0)));
                ...
                parsedObj.add(HDBSequenceConstants.DEPENDS_ON_TABLE_PROPERTY,
                        visitDependsOnTable((DependsOnTableContext) tree.getChild(0)));
    ...
    public JsonElement visitSchema(SchemaContext ctx) {
        ...
        return (ctx != null && ctx.STRING() != null) ? new JsonPrimitive(ctx.STRING()
                                                                            .getText())
                : null;
