...
public class XSODataDefinitionListener extends XSODataBaseListener {
        ...
        this.navEntries.forEach(el -> {
            ...
            if(el.fromend()!=null){
                if(el.fromend().principal()!=null){
                    navProp.setFromBindingType(XSODataBindingType.fromValue(el.fromend().principal().getText()).get());
                }
                if(el.fromend().dependent()!=null){
                    navProp.setFromBindingType(XSODataBindingType.fromValue(el.fromend().dependent().getText()).get());
                }
            }
    ...
    public void exitAssociation(AssociationContext ctx) {
        ...
        if (ctx.assoctable() != null) {
            ...
            if (ctx.assoctable().overprincipalend() != null) {
                associationTable.setPrincipal(assembleBindingRoleFromAssociationTableContext(ctx.assoctable().overprincipalend(), XSODataBindingType.PRINCIPAL));
            }
            // Set Dependent
            if (ctx.assoctable().overdependentend() != null) {
                associationTable.setDependent(assembleBindingRoleFromAssociationTableContext(ctx.assoctable().overdependentend(), XSODataBindingType.DEPENDENT));
            }
            // Set Events
        ...
        if (ctx.storage() != null) {
            ...
            if (ctx.storage().storageend() != null && XSODataStorageType.fromValue(ctx.storage().storageend().getText()).isPresent()) {
                storage.setStorageType(XSODataStorageType.fromValue(ctx.storage().storageend().getText()).get());
            }
    ...
    private XSODataBinding assembleBindingFromAssociationContext(ParserRuleContext context, XSODataBindingType type) {
        ...
        EndContext endContext = (EndContext) context.children.stream().filter(x -> x instanceof EndContext).findAny().orElse(null);
        if (endContext != null) {
            ...
            Optional<XSODataMultiplicityType> multiType = XSODataMultiplicityType.fromValue(handleStringLiteral(endContext.multiplicity().multiplicityvalue().getText()));
    ...
    private XSODataBindingRole assembleBindingRoleFromAssociationTableContext(ParserRuleContext context, XSODataBindingType type) {
        ...
        OverendContext endContext = (OverendContext) context.children.stream().filter(x -> x instanceof OverendContext).findAny().orElse(null);
        if (endContext != null) {
        ...
        modificationBody.modification().forEach(el -> {
            ...
            if (spec != null && spec.events() != null) {
                spec.events().eventlist().eventlistElement().forEach(eventListEl -> {
                    ...
                    Optional<XSODataEventType> eventType = XSODataEventType.fromValue(eventListEl.eventtype().getText());
