...
public class SymbolFactory {
    ...
    public DataTypeSymbol getDataTypeSymbol(DataTypeRuleContext ctx, Scope currentScope, String schema) {
        ...
        throw new CDSRuntimeException(
                String.format("Error at line: %s  - Invalid type provided: '%s' Data type syntax is only allowed for keyword 'type'",
                        ctx.artifactName.start.getLine(), ctx.artifactType.getText()));
    }
    ...
    public EntityElementSymbol getEntityElementSymbol(ElementDeclRuleContext ctx, Scope currentScope) {
        ...
        if (ctx.key != null && !ctx.key.getText()
            ...
            throw new CDSRuntimeException(
                    String.format("Error at line: %s  - Before an entity element declaration only the 'key' keyword is allowed",
                            ctx.key.start.getLine()));
        }
    ...
    public FieldSymbol getFieldSymbol(FieldDeclRuleContext ctx, Scope currentScope) {
        String filedId = HDBDDUtils.processEscapedSymbolName(ctx.identifier()
                                                                .getText());
    ...
    private SymbolTypeEnum parseToSymbolTypeEnum(Token artifactType) {
        try {
            return SymbolTypeEnum.valueOf(artifactType.getText()
                                                      .toUpperCase());
        } catch (IllegalArgumentException illegalArgumentException) {
            throw new CDSRuntimeException(String.format("Error at line: %s  - '%s' is not a valid artifact type.", artifactType.getLine(),
                    artifactType.getText()));
        }
    }
