...
import com.codbex.kronos.parser.hdbtable.exceptions.XSKHDBTableDuplicatePropertyException;
...
public class XSKTableParserTest extends AbstractDirigibleTest {
    ...
    public void parseTable() throws Exception {
        InputStream in = XSKTableParserTest.class.getResourceAsStream("/Events.hdbtable");
        String contents = IOUtils.toString(in, StandardCharsets.UTF_8);
        XSKDataStructureHDBTableModel model = XSKDataStructureModelFactory.parseTable("/Events.hdbtable", contents);

        assertEquals(6, model.getColumns().size());
        assertEquals("Events", model.getName());
        assertEquals("EVENTS", model.getSchema());
        assertEquals("COLUMNSTORE", model.getTableType());
        assertEquals("Team players", model.getDescription());
        assertEquals("/Events.hdbtable", model.getLocation());
        ...
        assertEquals("PK_Events", model.getConstraints().getPrimaryKey().getName());
    ...
    public void failIfParsingWrongPKDefinition() {
      String content = "table.schemaName = \"EVENTS\";\n" +
    ...
    public  void parseTableWithoutPK() throws Exception {
        String content = "table.schemaName  = \"DEMO\";\n" +
                "table.tableType   = COLUMNSTORE;\n" +
