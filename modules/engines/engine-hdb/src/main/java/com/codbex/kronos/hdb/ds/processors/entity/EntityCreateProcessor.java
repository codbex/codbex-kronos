 ...
 */
public class EntityCreateProcessor extends AbstractProcessor<DataStructureEntityModel> {

  private static final Logger logger = LoggerFactory.getLogger(EntityCreateProcessor.class);
  ...
  public boolean execute(Connection connection, DataStructureEntityModel entityModel) throws SQLException {
    ...
    List<HDBTableColumnDataStructureModel> columns = entityModel.getColumns();
    ...
    if (entityModel.getConstraints() != null) {
      ...
      if (entityModel.getConstraints().getForeignKeys() != null) {
        for (HDBTableConstraintForeignKeyDataStructureModel foreignKey : entityModel.getConstraints().getForeignKeys()) {
