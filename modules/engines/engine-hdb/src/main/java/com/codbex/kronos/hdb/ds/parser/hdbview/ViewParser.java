...
public class ViewParser implements DataStructureParser<HDBViewDataStructureModel> {
  ...
  @Override
  public HDBViewDataStructureModel parse(DataStructureParametersModel parametersModel)
  ...
  private HDBViewDataStructureModel parseHanaXSAdvancedContent(String location, String content) {
    logger.debug("Parsing hdbview as Hana XS Advanced format");
    HDBViewDataStructureModel hdbViewModel = new HDBViewDataStructureModel();
    HDBUtils
        .populateDataStructureModel(location, content, hdbViewModel, HDBDataStructureModel.TYPE_HDB_VIEW, DBContentType.OTHERS);
    hdbViewModel.setRawContent(content);
    return hdbViewModel;
  }

  private HDBViewDataStructureModel parseHanaXSClassicContent(String location, String content)
      throws DataStructuresException, IOException, ArtifactParserException {
    logger.debug("Parsing hdbview as Hana XS Classic format");
    HDBViewDataStructureModel hdbViewModel = new HDBViewDataStructureModel();
    HDBUtils
        .populateDataStructureModel(location, content, hdbViewModel, HDBDataStructureModel.TYPE_HDB_VIEW, DBContentType.XS_CLASSIC);
    ...
    if (antlr4Model.getDependsOn() != null) {
      antlr4Model.getDependsOn().forEach(el -> {
        DependencyDataStructureModel dependencyModel = new DependencyDataStructureModel(el, "none");
    ...
    if (antlr4Model.getDependsOnTable() != null) {
      antlr4Model.getDependsOnTable().forEach(el -> {
        DependencyDataStructureModel dependencyModel = new DependencyDataStructureModel(el, "TABLE");
        if (!hdbViewModel.getDependencies().contains(dependencyModel)) {
          hdbViewModel.getDependencies().add(dependencyModel);
        }
      });
    ...
    if (antlr4Model.getDependsOnView() != null) {
      antlr4Model.getDependsOnView().forEach(el -> {
        DependencyDataStructureModel dependencyModel = new DependencyDataStructureModel(el, "VIEW");
        if (!hdbViewModel.getDependencies().contains(dependencyModel)) {
          hdbViewModel.getDependencies().add(dependencyModel);
        }
      });
  ...
  public String getType() {
    return HDBDataStructureModel.TYPE_HDB_VIEW;
  }

  @Override
  public Class<HDBViewDataStructureModel> getDataStructureClass() {
    return HDBViewDataStructureModel.class;
  }
