 ...
 */
public class EntityForeignKeysProcessor extends AbstractProcessor<DataStructureEntityModel> {

  private static final Logger logger = LoggerFactory.getLogger(EntityForeignKeysProcessor.class);
...
//		CreateTableBuilder createTableBuilder = SqlFactory.getNative(connection).create().table(tableName);
    ...
    List<HDBTableConstraintForeignKeyDataStructureModel> foreignKeys = entityModel.getConstraints().getForeignKeys();
    for (HDBTableConstraintForeignKeyDataStructureModel foreignKeyModel : foreignKeys) {
      ...
      if (existing) {
        ...
        try {
          ...
          CommonsUtils.logProcessorErrors(ex.getMessage(), CommonsConstants.PROCESSOR_ERROR, entityModel.getLocation(),
              CommonsConstants.KRONOS_ENTITY_PROCESSOR);
          return false;
        }
      } else {
