 ...
 */
public class ProcedureCreateProcessor extends AbstractHDBProcessor<DataStructureHDBProcedureModel> {

  /** The Constant logger. */
  private static final Logger logger = LoggerFactory.getLogger(ProcedureCreateProcessor.class);
  ...
  public boolean execute(Connection connection, DataStructureHDBProcedureModel procedureModel)
    ...
    if (!SqlFactory.getNative(connection).exists(connection, procedureNameWithoutSchema, DatabaseArtifactTypes.PROCEDURE)) {
      ...
      if (!(dialect.getClass().equals(HanaSqlDialect.class))) {
        ...
        String sql = Constants.HDBPROCEDURE_CREATE + procedureModel.getRawContent();
        try {
