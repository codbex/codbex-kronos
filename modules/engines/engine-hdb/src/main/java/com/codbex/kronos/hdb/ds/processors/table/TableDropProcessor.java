 ...
 */
public class TableDropProcessor extends AbstractProcessor<HDBTableDataStructureModel> {
  ...
  public boolean execute(Connection connection, HDBTableDataStructureModel tableModel)
    ...
    if (SqlFactory.getNative(connection).exists(connection, tableName)) {
      ...
      if (tableModel.getConstraints().getForeignKeys() != null) {
        for (HDBTableConstraintForeignKeyDataStructureModel foreignKeyModel : tableModel.getConstraints().getForeignKeys()) {
  ...
  private boolean executeUpdate(Connection connection, String sql, HDBTableDataStructureModel tableModel)
      throws SQLException {
    PreparedStatement statement = null;
    try {
      statement = connection.prepareStatement(sql);
