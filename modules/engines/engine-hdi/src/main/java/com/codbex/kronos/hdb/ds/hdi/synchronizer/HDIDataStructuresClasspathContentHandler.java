 ...
 */
public class HDIDataStructuresClasspathContentHandler extends AbstractClasspathContentHandler {

  private static final Logger logger = LoggerFactory.getLogger(HDIDataStructuresClasspathContentHandler.class);

  private HDIDataStructuresSynchronizer dataStructuresHDISynchronizer = new HDIDataStructuresSynchronizer();
  ...
  protected boolean isValid(String path) {
    ...
    try {
      ...
      if (hdiSupported) {
        try {
          if (path.endsWith(HDBDataStructureModel.FILE_EXTENSION_HDI)) {
