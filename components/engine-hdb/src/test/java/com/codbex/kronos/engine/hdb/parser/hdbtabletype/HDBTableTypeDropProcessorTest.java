...
public class HDBTableTypeDropProcessorTest {
    ...
    public void testExecuteTableTypeDoesNotExist() throws SQLException {
        try (MockedStatic<SqlFactory> sqlFactory = Mockito.mockStatic(SqlFactory.class);
            ...
            sqlFactory.when(() -> SqlFactory.getNative(connectionMock))
            ...
            Mockito.verify(dropProcessorSpy)
                   .tableTypeDoesNotExist(connectionMock, model);
            // Mockito.verify(synonymRemoverMock).removePublicSynonym(connectionMock, model.getSchema(),
            // model.getName());
    ...
    public void testExecuteTableType() throws SQLException {
        try (MockedStatic<SqlFactory> sqlFactory = Mockito.mockStatic(SqlFactory.class);
            ...
            Mockito.doReturn(false)
                   .when(dropProcessorSpy)
                   .tableTypeDoesNotExist(connectionMock, model);
            Mockito.doReturn(ESCAPED_TABLE_TYPE_NAME)
                   .when(dropProcessorSpy)
                   .escapeTableTypeName(connectionMock, model);
            Mockito.doReturn(SQL_TO_DROP_TABLE_TYPE)
                   .when(dropProcessorSpy)
                   .getDropTableTypeSQL(connectionMock, ESCAPED_TABLE_TYPE_NAME);
            Mockito.doNothing()
                   .when(dropProcessorSpy)
                   .executeSql(SQL_TO_DROP_TABLE_TYPE, connectionMock);
    ...
    public void testExecuteNoTableTypeSupportWithSQLException() throws SQLException {
        try (MockedStatic<SqlFactory> sqlFactory = Mockito.mockStatic(SqlFactory.class);
            ...
            Mockito.doThrow(sqlException)
                   .when(dropProcessorSpy)
                   .executeSql(SQL_TO_DROP_TABLE_TYPE, connectionMock);
            // Mockito.doNothing().when(dropProcessorSpy).processException(model,
            // CommonsConstants.HDB_TABLE_TYPE_PARSER, sqlException);
            ...
            Mockito.verify(dropProcessorSpy)
                   .getDropTableTypeSQL(connectionMock, ESCAPED_TABLE_TYPE_NAME);
            // Mockito.verify(dropProcessorSpy).processException(model, CommonsConstants.HDB_TABLE_TYPE_PARSER,
            // sqlException);
            // Mockito.verify(synonymRemoverMock).removePublicSynonym(connectionMock, model.getSchema(),
            // model.getName());
            // Mockito.verifyNoMoreInteractions(connectionMock);
    ...
    public void testExecuteNoTableTypeSupportInDB() throws SQLException {
        try (MockedStatic<SqlFactory> sqlFactory = Mockito.mockStatic(SqlFactory.class);
            ...
            Mockito.doThrow(exception)
                   ...
                   .getDropTableTypeSQL(connectionMock, ESCAPED_TABLE_TYPE_NAME);
            // Mockito.doNothing().when(dropProcessorSpy).processException(model,
            // CommonsConstants.HDB_TABLE_TYPE_PARSER, exception);
            ...
            Mockito.verify(dropProcessorSpy)
                   .escapeTableTypeName(connectionMock, model);
            Mockito.verify(dropProcessorSpy, Mockito.times(0))
                   .executeSql(Mockito.any(String.class), Mockito.any(Connection.class));
            Mockito.verify(dropProcessorSpy)
                   .getDropTableTypeSQL(connectionMock, ESCAPED_TABLE_TYPE_NAME);
            // Mockito.verify(dropProcessorSpy).processException(model, CommonsConstants.HDB_TABLE_TYPE_PARSER,
            // exception);
