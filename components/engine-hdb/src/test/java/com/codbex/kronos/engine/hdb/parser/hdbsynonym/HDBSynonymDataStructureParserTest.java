...
public class HDBSynonymDataStructureParserTest {
    ...
    public void parseHdbsynonymFileWithoutErrorsSuccessfully() throws Exception {
        String hdbsynonymSample = org.apache.commons.io.IOUtils.toString(
                HDBSynonymDataStructureParserTest.class.getResourceAsStream("/MySynonym.hdbsynonym"), StandardCharsets.UTF_8);

        HDBSynonymGroup model = HDBDataStructureModelFactory.parseSynonym("hdb_view/MySynonym.hdbsynonym", hdbsynonymSample);

        HDBSynonym definitionModel = model.getSynonymDefinitions()
                                          .get("SY_DUMMY");
        assertEquals("hdb_view::MySynonym", model.getName());
        assertEquals("LL", definitionModel.getSchema());
        assertEquals("TABLE::CUSTOMERS", definitionModel.getTarget()
                                                        .getObject());
        assertEquals("DBADMIN", definitionModel.getTarget()
                                               .getSchema());
    }
    ...
    public void parseHdbsynonymFileMissingTargetSchemaFailed() throws Exception {
        ...
        HDBParameters parametersModel = new HDBParameters(null, "hdb_view/MySynonym.hdbsynonym", content, null);
