...
@Component
@Order(220)
public class HDBSynonymGroupsSynchronizer<A extends Artefact> implements Synchronizer<HDBSynonymGroup> {
	...
	private HDBSynonymGroupService synonymGroupService;
	 ...
	 */
	@Override
	public ArtefactService<HDBSynonymGroup> getService() {
...
//		Configuration.configureObject(synonymGroup);
	...
	static void reassignIds(HDBSynonymGroup synonymGroup, HDBSynonymGroup maybe) {
		synonymGroup.getSynonymDefinitions().values().forEach(s -> {
			...
			if (m != null) {
				s.setId(m.getId());
				s.getTarget().setId(m.getTarget().getId());
			}
		});
	}
	...
	public void setStatus(Artefact artefact, ArtefactLifecycle lifecycle, String error) {
		...
		getService().save((HDBSynonymGroup) artefact);
	}
	...
	public boolean complete(TopologyWrapper<Artefact> wrapper, ArtefactPhase flow) {
		...
		try (Connection connection = datasourcesManager.getDefaultDataSource().getConnection()) {
			...
			HDBSynonymGroup synonymGroup = null;
			if (wrapper.getArtefact() instanceof HDBSynonymGroup) {
				synonymGroup = (HDBSynonymGroup) wrapper.getArtefact();
			} else {
				throw new UnsupportedOperationException(String.format("Trying to process %s as HDBSynonymGroup", wrapper.getArtefact().getClass()));
			}
	 ...
	 * @param connection
	 *            the connection
	 * @param synonymGroupModel
	 *            the SynonymGroup model
	 * @throws SQLException
	 *             the SQL exception
	 */
