    ...
    public void execute(Connection connection, HDBTable tableModel) throws SQLException {
        ...
        if (SqlFactory.getNative(connection)
                ...
                if (resultSet.next()) {
                    ...
                    if (count > 0) {
                        String errorMessage = String.format(
                                "Drop operation for the non empty Table %s will not be executed. Delete all the records in the table first.",
                                tableName);
                        CommonsUtils.logProcessorErrors(errorMessage, CommonsConstants.PROCESSOR_ERROR, tableModel.getLocation(),
                                CommonsConstants.HDB_TABLE_PARSER);
            ...
            if (tableModel.getConstraints()
                          .getForeignKeys() != null) {
                for (HDBTableConstraintForeignKey foreignKeyModel : tableModel.getConstraints()
                                                                              .getForeignKeys()) {
                    sql = SqlFactory.getNative(connection)
                                    .drop()
