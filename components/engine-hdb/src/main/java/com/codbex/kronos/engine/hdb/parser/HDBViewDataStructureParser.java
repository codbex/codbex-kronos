...
public class HDBViewDataStructureParser implements HDBDataStructureParser<HDBView> {
     ...
     */
    @Override
    public HDBView parse(HDBParameters parametersModel) throws DataStructuresException, IOException, ArtifactParserException {
    ...
    private HDBView parseHanaXSClassicContent(String location, String content)
        ...
        try {
            ...
            CommonsUtils.logCustomErrors(location, CommonsConstants.PARSER_ERROR, "", "", e.getMessage(), CommonsConstants.EXPECTED_FIELDS,
                    CommonsConstants.HDB_VIEW_PARSER, CommonsConstants.MODULE_PARSERS, CommonsConstants.SOURCE_PUBLISH_REQUEST,
                    CommonsConstants.PROGRAM_KRONOS);
            // dataStructuresSynchronizer.applyArtefactState(CommonsUtils.getRepositoryBaseObjectName(location),location,
            // VIEW_ARTEFACT, ArtefactState.FAILED_CREATE, e.getMessage());
            throw new DataStructuresException(
                    String.format("Wrong format of HDB View: [%s] during parsing. [%s]", location, e.getMessage()));
        }
