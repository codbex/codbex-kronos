 ...
 */
public class SequenceUpdateProcessor extends AbstractHDBProcessor<DataStructureHDBSequenceModel> {

  /** The Constant logger. */
  private static final Logger logger = LoggerFactory.getLogger(SequenceUpdateProcessor.class);
  ...
  public boolean execute(Connection connection, DataStructureHDBSequenceModel sequenceModel) throws SQLException {
    ...
    switch (sequenceModel.getDBContentType()) {
      ...
      case OTHERS: {
        ...
        if (dialect.getClass().equals(HanaSqlDialect.class)) {
          sql = Constants.HDBSEQUENCE_ALTER + sequenceModel.getRawContent();
          break;
        } else {
          ...
          applyArtefactState(sequenceModel.getName(), sequenceModel.getLocation(), SEQUENCE_ARTEFACT, ArtefactState.FAILED_UPDATE,
              errorMessage);
    ...
    try {
      ...
      applyArtefactState(sequenceModel.getName(), sequenceModel.getLocation(), SEQUENCE_ARTEFACT, ArtefactState.SUCCESSFUL_UPDATE,
          message);
      return true;
