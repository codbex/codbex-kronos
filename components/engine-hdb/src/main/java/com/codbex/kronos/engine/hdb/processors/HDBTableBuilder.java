...
public class HDBTableBuilder {
  ...
  private void addTableIndicesToBuilder(HanaCreateTableBuilder sqlTableBuilder, HDBTable tableModel) {
    ...
    for (HDBTableIndex indexModel : indexes) {
      String name = caseSensitive
          ? HDBUtils.escapeArtifactName(indexModel.getName())
          : indexModel.getName();
    ...
    if (Objects.nonNull(foreignKeys)) {
      for (HDBTableConstraintForeignKey foreignKey : foreignKeys) {
        ...
        sqlTableBuilder.foreignKey(foreignKeyName, foreignKeyColumns, foreignKeyReferencedTable,
            HDBUtils.escapeArtifactName(foreignKey.getReferencedSchema()),
  ...
  protected void addUniqueIndicesToBuilder(AbstractTableBuilder builder, HDBTable tableModel) {
    List<HDBTableConstraintUnique> uniqueIndices = tableModel.getConstraints().getUniqueIndexes();
