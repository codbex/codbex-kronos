 ...
 */
public class SequenceDropProcessor extends AbstractHDBProcessor<DataStructureHDBSequenceModel> {

  /** The Constant logger. */
  private static final Logger logger = LoggerFactory.getLogger(SequenceDropProcessor.class);
  ...
  public boolean execute(Connection connection, DataStructureHDBSequenceModel sequenceModel) throws SQLException {
    ...
    if (SqlFactory.getNative(connection).exists(connection, hdbSequenceName, DatabaseArtifactTypes.SEQUENCE)) {
      ...
      switch (sequenceModel.getDBContentType()) {
        ...
        case OTHERS: {
          ...
          if (dialect.getClass().equals(HanaSqlDialect.class)) {
            sql = Constants.HDBSEQUENCE_DROP + sequenceModel.getRawContent();
            break;
          } else {
            ...
            applyArtefactState(sequenceModel.getName(), sequenceModel.getLocation(), SEQUENCE_ARTEFACT, ArtefactState.FAILED_DELETE,
                errorMessage);
      ...
      try {
        ...
        applyArtefactState(sequenceModel.getName(), sequenceModel.getLocation(), SEQUENCE_ARTEFACT, ArtefactState.SUCCESSFUL_DELETE,
            message);
        return true;
