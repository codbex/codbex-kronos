...
public class XSJSLibSynchronizerFileCleanerTest {
    ...
    // when(repositoryMock.getResource(testResourcePath +
    // generatedExportsExtension)).thenReturn(resourceMock);
    //
    // XSJSLibSynchronizerFileCleaner cleaner = new XSJSLibSynchronizerFileCleaner(repositoryMock);
    // cleaner.cleanup(testResourcePath);
    //
    // Mockito.verify(repositoryMock, Mockito.times(1)).getResource(testResourcePath +
    // generatedExportsExtension);
