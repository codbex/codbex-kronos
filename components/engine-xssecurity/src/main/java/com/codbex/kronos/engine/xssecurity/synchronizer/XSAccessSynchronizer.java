...
public class XSAccessSynchronizer<A extends Artefact> implements Synchronizer<XSAccess> {
     ...
     */
    private XSAccessService xssecurityAccessService;
     ...
     */
    @Override
    public ArtefactService<XSAccess> getService() {
     ...
     * @return the service
     */
    public ArtefactService<Access> getAccessService() {
    ...
    public List<XSAccess> parse(String location, byte[] content) throws ParseException {
    	XSAccess xsaccess = XSAccessParser.parse(content).toAccessDefinition();
    	xsaccess.setLocation(location);
    	xsaccess.setName(Paths.get(location).getFileName().toString());
		...
		for (String role : xsaccess.getAuthorizationRoles()) {
	    	...
	    	if (xsaccess.getAuthenticationMethods() != null) {
	    		access.setMethod(xsaccess.getAuthenticationMethods().get(0)); // used for HTTP Method as Authentication Method is not supported anyway
	    	} else {
	    		access.setMethod("*");
	    	}
	    	access.setRole(role);
	...
	public void setStatus(Artefact artefact, ArtefactLifecycle lifecycle, String error) {
		...
		getService().save((XSAccess) artefact);
	}
     ...
     */
    @Override
    public void cleanup(XSAccess xsaccess) {
        try {
