...
class ODataNamesValidationPatternPatcher {
	...
	void patch() throws ReflectiveOperationException {
		setFinalStatic(EdmNamedImplProv.class.getDeclaredField(NAME_OF_THE_PROPERTY_NAME_PATTERN_FIELD),
				ODATA_PROPERTY_NAME_PATTERN);
	}

	private static void setFinalStatic(Field field, Object newValue)
			throws InvocationTargetException, NoSuchMethodException, IllegalAccessException, NoSuchFieldException, SecurityException {
	    ...
	    unsafe.putObject(field.getDeclaringClass(), unsafe.staticFieldOffset(field), newValue);
	}
	...
	private static Field getModifiersField()
		...
		return Arrays.stream(allDeclaredFields).filter(f -> "modifiers".equals(f.getName())).findFirst()
