...
import static com.codbex.kronos.engine.xsodata.handler.AbstractKronosOData2EventHandler.AFTER_TABLE_NAME_CONTEXT_KEY;
import static com.codbex.kronos.engine.xsodata.handler.AbstractKronosOData2EventHandler.BEFORE_TABLE_NAME_CONTEXT_KEY;
import static com.codbex.kronos.engine.xsodata.handler.AbstractKronosOData2EventHandler.CONNECTION_CONTEXT_KEY;
import static com.codbex.kronos.engine.xsodata.handler.AbstractKronosOData2EventHandler.DATASOURCE_CONTEXT_KEY;
import static com.codbex.kronos.engine.xsodata.handler.AbstractKronosOData2EventHandler.ENTRY_JSON_CONTEXT_KEY;
import static com.codbex.kronos.engine.xsodata.handler.AbstractKronosOData2EventHandler.SQL_BUILDER_CONTEXT_KEY;
...
import com.codbex.kronos.engine.xsodata.handler.KronosOData2EventHandler;
import com.codbex.kronos.engine.xsodata.handler.KronosProcedureOData2EventHandler;
import com.codbex.kronos.engine.xsodata.handler.KronosScriptingOData2EventHandler;
...
public class KronosOData2EventHandlerTest {
    ...
    public void setup() {
        ...
        staticObjects.when(() -> StaticObjects.get(StaticObjects.DATASOURCE))
                     .thenReturn(dataSource, dataSource);
    ...
    public void testBeforeCreateEntity() throws org.apache.olingo.odata2.api.exception.ODataException, ODataException, SQLException {
        ...
        Mockito.doNothing()
               .when(spyScriptingHandler)
               .callSuperBeforeCreateEntity(any(), any(), any(), any(), any());
    ...
    public void testAfterCreateEntity() throws ODataException, org.apache.olingo.odata2.api.exception.ODataException, SQLException {
        ...
        ODataResponse response =
                kronosOData2EventHandler.afterCreateEntity(uriInfo, "application/json", "application/json", entry, procedureHandlerContext);
        assertResponse(response);
        ...
        Mockito.doNothing()
               .when(spyScriptingHandler)
               .callSuperAfterCreateEntity(any(), any(), any(), any(), any());
    ...
    public void testOnCreateEntity() throws ODataException, org.apache.olingo.odata2.api.exception.ODataException, SQLException {
        ...
        Mockito.doNothing()
               .when(spyScriptingHandler)
               .callSuperOnCreateEntity(any(), any(), any(), any(), any());
    ...
    public void testBeforeUpdateEntity() throws ODataException, org.apache.olingo.odata2.api.exception.ODataException, SQLException {
        ...
        Mockito.doNothing()
               .when(spyScriptingHandler)
               .callSuperBeforeUpdateEntity(any(), any(), anyBoolean(), any(), any(), any());
    ...
    public void testAfterUpdateEntity() throws SQLException, ODataException, org.apache.olingo.odata2.api.exception.ODataException {
        ...
        Mockito.doNothing()
               .when(spyScriptingHandler)
               .callSuperAfterUpdateEntity(any(), any(), anyBoolean(), any(), any(), any());
    ...
    public void testOnUpdateEntity() throws SQLException, ODataException, org.apache.olingo.odata2.api.exception.ODataException {
        ...
        Mockito.doNothing()
               .when(spyScriptingHandler)
               .callSuperOnUpdateEntity(any(), any(), any(), anyBoolean(), any(), any());
        kronosOData2EventHandler.onUpdateEntity(uriInfo, inputStream, "application/json", true, "application/json",
                scriptingHandlerContext);
        assertTrue(scriptingHandlerContext.containsKey(CONNECTION_CONTEXT_KEY));
        assertTrue(scriptingHandlerContext.containsKey(BEFORE_TABLE_NAME_CONTEXT_KEY));
        assertTrue(scriptingHandlerContext.containsKey(AFTER_TABLE_NAME_CONTEXT_KEY));
    }
    ...
    public void testBeforeDeleteEntity() throws SQLException, ODataException, org.apache.olingo.odata2.api.exception.ODataException {
        ...
        Mockito.doNothing()
               .when(spyScriptingHandler)
               .callSuperBeforeDeleteEntity(any(), any(), any());
    ...
    public void testAfterDeleteEntity() throws SQLException, ODataException, org.apache.olingo.odata2.api.exception.ODataException {
        ...
        Mockito.doNothing()
               .when(spyScriptingHandler)
               .callSuperAfterDeleteEntity(any(), any(), any());
    ...
    public void testOnDeleteEntity() throws SQLException, ODataException, org.apache.olingo.odata2.api.exception.ODataException {
        ...
        Mockito.doNothing()
               .when(spyScriptingHandler)
               .callSuperOnDeleteEntity(any(), any(), any());
    ...
    private void mockGetHandlers() throws EdmException, ODataException {
        Mockito.when(uriInfo.getTargetType())
               .thenReturn(edmType, edmType);
        Mockito.when(edmType.getNamespace())
               .thenReturn("test-namespace", "test-namespace");
        Mockito.when(edmType.getName())
               .thenReturn("test-name", "test-name");
        ...
        Mockito.when(ODataHandlerService.get()
                                        .getByNamespaceNameMethodAndKind(any(), any(), any(), any()))
    ...
    private void mockTemporaryTables(AbstractKronosOData2EventHandler handler)
        ...
        Mockito.doReturn("test-table")
               .when(handler)
               .getSQLInsertBuilderTargetTable(any(), any());

        Mockito.doReturn("test-table")
               .when(handler)
               .getSQLUpdateBuilderTargetTable(any(), any());

        Mockito.doReturn("test-table")
               .when(handler)
               .getSQLDeleteBuilderTargetTable(any(), any());

        Mockito.doNothing()
               .when(handler)
               .createTemporaryTableLikeTable(any(), any(), any());

        Mockito.doNothing()
               .when(handler)
               .createTemporaryTableAsSelect(any(), any(), any(), any());

        Mockito.doNothing()
               .when(handler)
               .insertIntoTemporaryTable(any(), any(), any(), any());

        Mockito.doNothing()
               .when(handler)
               .updateTemporaryTable(any(), any(), any(), any());

        Mockito.doReturn(new HashMap<>())
               .when(handler)
               .readEntryMap(any(), any());

        if (handler instanceof KronosProcedureOData2EventHandler) {
            Mockito.doReturn("TEST_SCHEMA")
                   .when((KronosProcedureOData2EventHandler) handler)
                   .getODataArtifactTypeSchema("test-table");
        }
    }
     ...
     * @throws SQLException the SQL exception
     */
    private void mockCallProcedure(KronosProcedureOData2EventHandler handler, boolean isUpdate) throws SQLException {
        ResultSet resultSetMock = Mockito.mock(ResultSet.class);
        Mockito.when(resultSetMock.next())
               .thenReturn(true);
        Mockito.when(resultSetMock.getString(anyString()))
               .thenReturn("400", "INVALID ID");
        Mockito.when(resultSetMock.getString(anyInt()))
               .thenReturn("detail message 1", "detail message 2");
        ...
        Mockito.when(metaData.getColumnName(anyInt()))
               .thenReturn("errorDetail1", "errorDetail2");

        Mockito.when(resultSetMock.getMetaData())
               .thenReturn(metaData);

        if (isUpdate) {
            Mockito.doReturn(resultSetMock)
                   .when(handler)
                   .callProcedure(any(), any(), any(), any(), any());
        } else {
            Mockito.doReturn(resultSetMock)
                   .when(handler)
                   .callProcedure(any(), any(), any(), any());
        }
    }
    ...
    private void assertResponse(ODataResponse response) throws EntityProviderException {
        ...
        Map<String, Object> responseBody =
                GsonHelper.fromJson(new BasicEntityProvider().readText(((InputStream) response.getEntity())), HashMap.class);
