...
public class GrantPrivilegesExternalArtifactsSchemaProcessor extends HDIAbstractProcessor {
    ...
    private Set<String> collectExternalArtifactsSchemas(String[] deploys) throws IOException, DataStructuresException {
        ...
        for (String deploy : deploys) {
            if (FilenameUtils.getExtension(deploy)
                             .equalsIgnoreCase(HDBSYNONYM_FILE_EXTENSION)) {
                String hdbSynonymContent = getSynonymContent(deploy);
                HDBSynonymGroup synonymModel = getSynonymModel(deploy, hdbSynonymContent);

                for (Entry<String, HDBSynonym> synonymDefinition : synonymModel.getSynonymDefinitions()
                                                                               .entrySet()) {
                    String externalArtifactSchema = synonymDefinition.getValue()
                                                                     .getTarget()
                                                                     .getSchema();
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     */
    public String getSynonymContent(String deploy) throws IOException {
    ...
    private void grantPrivileges(Connection connection, String container, Set<String> externalArtifactsSchemas) throws SQLException {
        ...
        for (String externalSchema : externalArtifactsSchemas) {
            executeUpdate(connection,
                    format("GRANT SELECT ON SCHEMA \"{0}\" TO \"{1}\" WITH GRANT OPTION;", externalSchema, containerOwner));
            executeUpdate(connection,
                    format("GRANT EXECUTE ON SCHEMA \"{0}\" TO \"{1}\" WITH GRANT OPTION;", externalSchema, containerOwner));
            executeUpdate(connection,
                    format("GRANT INSERT ON SCHEMA \"{0}\" TO \"{1}\" WITH GRANT OPTION;", externalSchema, containerOwner));
            executeUpdate(connection,
                    format("GRANT UPDATE ON SCHEMA \"{0}\" TO \"{1}\" WITH GRANT OPTION;", externalSchema, containerOwner));
            executeUpdate(connection,
                    format("GRANT DELETE ON SCHEMA \"{0}\" TO \"{1}\" WITH GRANT OPTION;", externalSchema, containerOwner));
        }
    }
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     */
    public HDBSynonymGroup getSynonymModel(String location, String content) throws DataStructuresException, IOException {
        return HDBDataStructureModelFactory.parseSynonym(location, content);
    }
}
