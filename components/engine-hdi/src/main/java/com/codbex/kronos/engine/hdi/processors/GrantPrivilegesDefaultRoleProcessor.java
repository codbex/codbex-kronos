...
import com.codbex.kronos.engine.hdi.exceptions.GrantPrivilegesSQLException;
...
public class GrantPrivilegesDefaultRoleProcessor extends HDIAbstractProcessor {
    ...
    public void execute(Connection connection, String container, String user, String[] deployPaths) {
        ...
        Arrays.stream(deployPaths)
              .filter(deployPath -> deployPath.endsWith("kronos_technical_privileges.hdbrole"))
              .forEach(filePath -> {
                  try {
                      grantPrivileges(connection, container, user);
                  } catch (SQLException exception) {
                      throw new GrantPrivilegesSQLException("Failed to grant kronos_technical_privileges.hdbrole to user " + user,
                              exception);
                  }
              });
