...
public class CloudPlatformDestinationFacade {
    ...
    public static Destination getDestination(String destinationName) {
        ...
        if (fetchedDestination.isHttp()) {
            URI uri = URI.create((String) fetchedDestination.get("URL")
                                                            .get());
    ...
    public static String executeRequest(String requestObject, String destinationName, String options)
        ...
        HttpDestination httpDestination = DestinationAccessor.getDestination(destinationName)
                                                             .asHttp();
        HttpClient client = HttpClientAccessor.getHttpClient(httpDestination);
        String uri = URI.create(httpDestination.getUri() + destinationRequest.getPath())
                        .toString();
        ...
        Header[] headers = Arrays.stream(response.getAllHeaders())
                                 .map(CloudPlatformDestinationFacade::createHeader)
                                 .toArray(Header[]::new);

        DestinationResponse destinationResponse = new DestinationResponse();
        destinationResponse.setHeaders(headers);
        destinationResponse.setStatusCode(response.getStatusLine()
                                                  .getStatusCode());
