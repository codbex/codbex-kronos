...
public class HDBViewHanaITest extends AbstractHDBITest {
    ...
    public void testHDBViewCreateOnSameSchema() throws Exception {
        try (Connection connection = datasource.getConnection(); Statement stmt = connection.createStatement()) {
            ...
            try {
                stmt.executeUpdate(
                        String.format("create table \"%s\".\"acme.com.test.tables::MY_TABLE10\"(COLUMN1 integer,COLUMN2 integer)",
                                userSchema));

                stmt.executeUpdate(
                        String.format("create table \"%s\".\"acme.com.test.tables::MY_TABLE20\"(COLUMN1 integer,COLUMN2 integer)",
                                userSchema));
    ...
    public void testHDBViewUpdateOnSameSchema() throws Exception {
        try (Connection connection = datasource.getConnection(); Statement stmt = connection.createStatement()) {
            ...
            try {
                ...
                stmt.executeUpdate(
                        String.format("create VIEW  \"%s\".\"%s\" AS select * from \"acme.com.test.tables::MY_TABLE10\"", userSchema,
                                artifactName));

                stmt.executeUpdate(
                        String.format("create SYNONYM \"%s\".\"%s\" FOR \"%s\".\"%s\"", Constants.SYNONYM_PUBLIC_SCHEMA, artifactName,
    ...
    public void testHDBViewCreateOnDiffSchemasWithExistingSynonym() throws Exception {
        try (Connection connection = datasource.getConnection(); Statement stmt = connection.createStatement()) {
            ...
            try {
                ...
                stmt.executeUpdate(
                        String.format("CREATE VIEW \"%s\".\"%s\" AS select * from \"%s\".\"acme.com.test.tables::MY_TABLE20\"", TEST_SCHEMA,
                                artifactName, TEST_SCHEMA));

                stmt.executeUpdate(
                        String.format("create SYNONYM \"%s\".\"%s\" FOR \"%s\".\"%s\"", Constants.SYNONYM_PUBLIC_SCHEMA, artifactName,
                                TEST_SCHEMA, artifactName));
