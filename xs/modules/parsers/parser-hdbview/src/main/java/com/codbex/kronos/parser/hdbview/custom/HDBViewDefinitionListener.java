...
/**
 * The listener interface for receiving HDBViewCore events. The class that is interested in
 * processing a HDBViewCore event implements this interface, and the object created with that class
 * is registered with a component using the component's <code>addHDBViewCoreListener</code> method.
 * When the HDBViewCore event occurs, that object's appropriate method is invoked.
 *
 */
public class HDBViewDefinitionListener extends HDBViewBaseListener {
     ...
     */
    @Override
    public void exitHdbViewDefinition(HdbViewDefinitionContext ctx) {
        ctx.property()
           .forEach(node -> {
               if (node.schemaProp() != null) {
                   model.setSchema(handleStringLiteral(node.schemaProp()
                                                           .STRING()
                                                           .getText()));
               }
               if (node.queryProp() != null) {
                   model.setQuery(handleStringLiteral(node.queryProp()
                                                          .STRING()
                                                          .getText()));
               }
               if (node.dependsOnProp() != null) {
                   model.setDependsOn(node.dependsOnProp()
                                          .STRING()
                                          .stream()
               ...
               if (node.dependsOnTable() != null) {
                   model.setDependsOnTable(node.dependsOnTable()
                                               .STRING()
                                               .stream()
                                               .map(el -> handleStringLiteral(el.getText()))
                                               .collect(Collectors.toList()));
               ...
               if (node.dependsOnView() != null) {
                   model.setDependsOnView(node.dependsOnView()
                                              .STRING()
                                              .stream()
                                              .map(el -> handleStringLiteral(el.getText()))
                                              .collect(Collectors.toList()));
               ...
               if (node.publicProp() != null) {
                   model.setPublic(Boolean.parseBoolean(node.publicProp()
                                                            .getText()));
               } else {
