...
public class HDBViewDefinitionListenerTest {
    ...
    public void testParseHDBViewFileSuccessfully() throws Exception {
        String hdbViewSample = org.apache.commons.io.IOUtils.toString(
                HDBViewDefinitionListenerTest.class.getResourceAsStream("/sample.hdbview"), StandardCharsets.UTF_8);
        ...
        assertEquals(2, model.getDependsOn()
                             .size());
        assertEquals("acme.com.test.tables::MY_TABLE1", model.getDependsOn()
                                                             .get(0));
        assertEquals("acme.com.test.views::MY_VIEW1", model.getDependsOn()
                                                           .get(1));
        assertEquals(2, model.getDependsOnTable()
                             .size());
        assertEquals("acme.com.test.tables::MY_TABLE1", model.getDependsOnTable()
                                                             .get(0));
        assertEquals("acme.com.test.views::MY_TABLE2", model.getDependsOnTable()
                                                            .get(1));
        assertEquals(2, model.getDependsOnView()
                             .size());
        assertEquals("acme.com.test.tables::MY_VIEW1", model.getDependsOnView()
                                                            .get(0));
        assertEquals("acme.com.test.views::MY_VIEW2", model.getDependsOnView()
                                                           .get(1));
        assertEquals(
    ...
    public void testParseHDBviewFileWithSyntaxErrorExceptionThrown() throws Exception {
        String hdbViewSample = org.apache.commons.io.IOUtils.toString(
                HDBViewDefinitionListenerTest.class.getResourceAsStream("/sample_with_errors.hdbview"), StandardCharsets.UTF_8);
    ...
    public void testParseHDBViewFileWithSingleQuote() throws Exception {
        String hdbViewSample = org.apache.commons.io.IOUtils.toString(
                HDBViewDefinitionListenerTest.class.getResourceAsStream("/sampleWithQuote.hdbview"), StandardCharsets.UTF_8);
        ...
        assertEquals(osLineSeparator + "-- some comment" + osLineSeparator + "select distinct code from" + osLineSeparator + "("
                + osLineSeparator + "    select current_status_code as code, status_model_code" + osLineSeparator
                + "    from \"sap.db.status::t_status_model_transition\"" + osLineSeparator + "    union all" + osLineSeparator
                + "    select next_status_code as code, status_model_code" + osLineSeparator
                + "    from \"sap.db.status::t_status_model_transition\"" + osLineSeparator + ")" + osLineSeparator
