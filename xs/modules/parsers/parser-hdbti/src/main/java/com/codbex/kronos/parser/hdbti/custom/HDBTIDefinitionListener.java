...
/**
 * The listener interface for receiving HDBTICore events. The class that is interested in processing
 * a HDBTICore event implements this interface, and the object created with that class is registered
 * with a component using the component's <code>addHDBTICoreListener</code> method. When the
 * HDBTICore event occurs, that object's appropriate method is invoked.
 *
 */
public class HDBTIDefinitionListener extends HDBTIBaseListener {
    ...
    public void exitObjConfig(ObjConfigContext ctx) {
        ...
        for (AssignExpressionContext expressionContext : ctx.assignExpression()) {
            if (expressionContext.assignTable() != null) {
                ...
                String distinguishEmptyFromNull = expressionContext.assignDistinguishEmptyFromNull()
                                                                   .BOOLEAN()
                                                                   .getText();
                ...
                expressionContext.assignKeys()
                                 .keyArr()
                                 .pair()
                                 .forEach(el -> {
                                     if (pairs.isEmpty()) {
                                         HDBTIImportConfigModel.Pair newPair = new HDBTIImportConfigModel.Pair(
                                                 handleStringLiteral(el.pairKey()
                                                                       .getText()),
                                                 new ArrayList<>(Collections.singletonList(handleStringLiteral(el.pairValue()
                                                                                                                 .getText()))));
                                         pairs.add(newPair);
                                     } else {
                                         for (HDBTIImportConfigModel.Pair pair : pairs) {
                                             if (pair.getColumn()
                                                     .equals(handleStringLiteral(el.pairKey()
                                                                                   .getText()))) {
                                                 pair.getValues()
                                                     .add(handleStringLiteral(el.pairValue()
                                                                                .getText()));
                                             } else {
                                                 HDBTIImportConfigModel.Pair newPair = new HDBTIImportConfigModel.Pair(
                                                         handleStringLiteral(el.pairKey()
    ...
    public void enterAssignExpression(AssignExpressionContext ctx) {
        String currentObjField = ctx.getChild(0)
                                    .getChild(0)
                                    .getText();
        ...
        if (usedFields.contains(currentObjField)) {
            if (currentObjField.equals("table")) {
                throw new DuplicateFieldNameException(String.format(
                        "Invalid combination of table declarations found, you may only use one of [cdstable | hdbtable | table]."));
            } else {
                throw new DuplicateFieldNameException(
                        String.format("Duplicate fields: Field with name: '%s' is already in use.", currentObjField));
            }
        } else {
    ...
    public void enterAssignTable(AssignTableContext ctx) {
        String tableProperty = handleStringLiteral(ctx.STRING()
                                                      .getText());
