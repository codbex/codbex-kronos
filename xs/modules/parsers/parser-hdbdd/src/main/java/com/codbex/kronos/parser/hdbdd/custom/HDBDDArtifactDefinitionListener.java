...
/**
 * The listener interface for receiving artifactDefinition events. The class that is interested in
 * processing a artifactDefinition event implements this interface, and the object created with that
 * class is registered with a component using the component's
 * <code>addArtifactDefinitionListener</code> method. When the artifactDefinition event occurs, that
...
public class HDBDDArtifactDefinitionListener extends CdsBaseListener {
    ...
    public void enterNamespaceRule(NamespaceRuleContext ctx) {
        String packageName = ctx.members.stream()
    ...
    public void enterUsingRule(UsingRuleContext ctx) {
        String packagePath = ctx.pack.stream()
                                     .map(IdentifierContext::getText)
                                     .map(HDBDDUtils::processEscapedSymbolName)
                                     .collect(Collectors.joining("."));
        String memberPath = ctx.members.stream()
                                       .map(IdentifierContext::getText)
                                       .map(HDBDDUtils::processEscapedSymbolName)
                                       .collect(Collectors.joining("."));
    ...
    public void enterAssociation(AssociationContext ctx) {
        ...
        if (ctx.key != null && ctx.key.getText()
    ...
    public void enterMinMaxCardinality(MinMaxCardinalityContext ctx) {
        AssociationSymbol associationSymbol = this.associations.get(ctx.getParent());
        int firstDotIndex = ctx.ASSOCIATION_MIN()
                               .getText()
                               .indexOf(".");
        String minValue = ctx.ASSOCIATION_MIN()
                             .getText()
    ...
    public void enterAssignType(AssignTypeContext ctx) {
        String fullReference = ctx.pathSubMembers.stream()
                                                 .map(IdentifierContext::getText)
                                                 .map(HDBDDUtils::processEscapedSymbolName)
                                                 .collect(Collectors.joining("."));
    ...
    public void exitAnnObjectRule(AnnObjectRuleContext ctx) {
        ...
        if (expectedAnnKeys == 0) {
            ...
            throw new CDSRuntimeException(
                    String.format("Error at line: %d col: %d. Annotation %s already assigned for %s.", ctx.identifier().start.getLine(),
                            ctx.identifier().start.getCharPositionInLine(), annId, symbolToBeAssigned.getFullName()));
            ...
            AbstractAnnotationValue expectedFieldValue = expectedValue.getKeyValuePairs()
                                                                      .values()
                                                                      .stream()
            ...
            Optional<String> expectedValueKeySet = expectedValue.getKeyValuePairs()
                                                                .keySet()
                                                                .stream()
                                                                .findFirst();
    ...
    public void exitAnnMarkerRule(AnnMarkerRuleContext ctx) {
        ...
        if (annId.equals(NOKEY_ANNOTATION)) {
            for (int i = 0; i <= ctx.getParent().children.size(); i++) {
                if (ctx.getParent()
                       .getChild(i) instanceof ElementDeclRuleContext && ctx.getParent()
                                                                            .getChild(i)
                                                                            .getChild(0)
                                                                            .getText()
                                                                            .equalsIgnoreCase("key")) {
                    throw new CDSRuntimeException(
                            String.format("Error at line: %d col: %d. Annotation %s has been specified for entity with keys.",
                                    annIdToken.start.getLine(), annIdToken.start.getCharPositionInLine(), expectedAnnObject.getName()));
                }
            }
        }
    ...
    public void enterSelectRule(SelectRuleContext ctx) {
        ...
        if (selectedColumnsRuleContext.children != null) {
            ...
            selectSymbol.setColumnsSql(ctx.start.getInputStream()
        ...
        if (whereRuleContext != null) {
            ...
            selectSymbol.setWhereSql(ctx.start.getInputStream()
                                              .getText(whereRuleSqlInterval));
        }
        ...
        if (ctx.dependsOnTable != null) {
            String parsedDependsOnTable = ctx.dependsOnTable.stream()
    ...
    public void enterJoinRule(JoinRuleContext ctx) {
        ...
        if (joinFieldsContext.children != null) {
            ...
            joinSymbol.setJoinFields(ctx.start.getInputStream()
                                              .getText(joinFieldsSqlInterval));
        }
    ...
    public void enterEnumRule(EnumRuleContext ctx) {
        ...
        annotationEnum.setValue(ctx.identifier()
                                   .ID()
                                   .getText());
    ...
    public void exitArrRule(ArrRuleContext ctx) {
        ...
        ctx.annValue()
           .forEach(valueCtx -> annotationArray.addValue(this.values.get(valueCtx)));
    ...
    public void exitObj(ObjContext ctx) {
        ...
        for (CdsParser.KeyValueContext keyValueCtx : ctx.keyValue()) {
            String key = keyValueCtx.identifier()
                                    .ID()
                                    .getText();
    ...
    public Symbol resolveReference(String referencedId, Symbol referencingSymbol) {
        if (symbolTable.getHanaTypeScope(referencedId.toUpperCase()) != null) {
            ...
            return symbolTable.getCDSTypeScope()
                              .resolve(cdsType);
        } else if (symbolTable.getCDSTypeScope()
                              .resolve(referencedId) != null) {
            return symbolTable.getCDSTypeScope()
    ...
    private void validateAnnotation(IdentifierContext annIdToken, Symbol symbol) {
        ...
        if (annotationObj == null) {
            throw new CDSRuntimeException(String.format("Error at line: %d col: %d. Annotation with name: %s not supported.",
                    annIdToken.start.getLine(), annIdToken.start.getCharPositionInLine(), annId));
        }

        if (annotationObj.getAllowedForSymbols()
                         .stream()
                         .filter(t -> symbol.getClass()
                                            .equals(t))
                         .findAny()
                         .isEmpty()) {
            throw new CDSRuntimeException(String.format("Error at line: %d col: %d. Annotation %s not allowed for %s",
                    annIdToken.start.getLine(), annIdToken.start.getCharPositionInLine(), annotationObj.getName(), symbol.getFullName()));
        } else if (annotationObj.isTopLevel() && !(symbol.getScope() instanceof CDSFileScope)) {
            throw new CDSRuntimeException(String.format("Error at line: %d col: %d. Annotation %s is only allowed for top level entities.",
                    annIdToken.start.getLine(), annIdToken.start.getCharPositionInLine(), annotationObj.getName()));
        }
    }
    ...
    private void assignBuiltInType(Symbol builtInType, ParserRuleContext ctx) {
        ...
        BuiltInTypeSymbol builtInTypeToProvide =
                new BuiltInTypeSymbol(builtInType.getName(), resolvedBuiltInType.getArgsCount(), resolvedBuiltInType.getValueType());
        Typeable typeable = typeables.get(ctx.getParent());
    ...
    private void assignBuiltInTypeWithArgs(Symbol builtInType, List<Token> args, Token typeIdToken, ParserRuleContext ctx) {
        ...
        args.forEach(t -> builtInTypeToProvide.addArgValue(Integer.parseInt(t.getText())));
    ...
    private void compareAnnValues(AbstractAnnotationValue providedValue, AbstractAnnotationValue expectedValue) {
        ...
        if (providedValue instanceof AnnotationObj) {
            ...
            Set<String> keys = expectedObj.getKeyValuePairs()
                                          .keySet();
            keys.forEach(k -> {
            ...
            providedArray.getValues()
                         .forEach(v -> compareAnnValues(v, expectedArrayValueType));
    ...
    private void registerSymbolToSymbolTable(Symbol symbol) {
        ...
        if (fullSymbolNames.isEmpty()) {
            ...
            fullName = fullSymbolNames.stream()
                                      .collect(Collectors.joining("."))
                    + "." + symbol.getName();
        }
    ...
    private void validateTopLevelSymbol(Symbol symbol) {
        ...
        if (schemaAnnotation == null) {
            throw new CDSRuntimeException(
                    String.format("Error at line: %s. Missing '@Schema' annotation for top level symbol definition: %s",
        ...
        if (!symbol.getName()
                   .equals(getTopLevelSymbolExpectedName())) {
    ...
    private String getExpectedNamespace() {
        String[] splitFileLocation = this.fileLocation.split("/");
        splitFileLocation = Arrays.stream(splitFileLocation)
                                  .filter(s -> !s.isEmpty())
                                  .toArray(String[]::new);
