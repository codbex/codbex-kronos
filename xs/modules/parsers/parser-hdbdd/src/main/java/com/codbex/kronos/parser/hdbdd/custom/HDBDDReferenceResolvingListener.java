 ...
 * processing a referenceResolving event implements this interface, and the object created with that
 * class is registered with a component using the component's
 * <code>addReferenceResolvingListener</code> method. When the referenceResolving event occurs, that
...
public class HDBDDReferenceResolvingListener extends CdsBaseListener {
    ...
    public void enterUsingRule(UsingRuleContext ctx) {
        ...
        if (externalSymbol == null) {
            throw new CDSRuntimeException(String.format("Error at line: %d col: %d. Non existing entity in package: %s::%s",
    ...
    public void enterAssignType(AssignTypeContext ctx) {
        ...
        Symbol resolvedTypeSymbol =
                resolveReferenceChain(referencingSymbol.getReference(), (Symbol) referencingSymbol, nonResolvedRefSymbols);
    ...
    public void enterDefaultValue(DefaultValueContext ctx) {
        ...
        BuiltInTypeSymbol typeOfElement = (BuiltInTypeSymbol) this.entityElements.get(ctx.getParent()
                                                                                         .getParent())
                                                                                 .getType();
    ...
    public void enterAssociationTarget(AssociationTargetContext ctx) {
        ...
        String reference = ctx.pathSubMembers.stream()
                                             .map(IdentifierContext::getText)
                                             .map(HDBDDUtils::processEscapedSymbolName)
                                             .collect(Collectors.joining("."));
        ...
        if (!(resolvedSymbol instanceof EntitySymbol) && resolvedSymbol != null) {
            throw new CDSRuntimeException(String.format("Error at line: %d col: %d. The provided reference must be an Entity!",
    ...
    public void exitAssociation(AssociationContext ctx) {
        ...
        if (ctx.getChild(ManagedForeignKeysContext.class, 0) == null && ctx.getChild(UnmanagedForeignKeyContext.class, 0) == null
                && associationSymbol.getTarget() != null) {
            List<EntityElementSymbol> associationKeys = associationSymbol.getTarget()
                                                                         .getKeys();
    ...
    public void enterUnmanagedForeignKey(UnmanagedForeignKeyContext ctx) {
        ...
        String targetFullName = associationSymbol.getTarget()
                                                 .getName();
        String reference = ctx.pathSubMembers.stream()
                                             .skip(1)
                                             .map(IdentifierContext::getText)
        ...
        if (resolvedSourcePkElement == null || !resolvedSourcePkElement.isKey()) {
            throw new CDSRuntimeException(
                    String.format("Error at line: %s. No such element: %s found in: %s or the element is not a primary key.",
                            ctx.source.start.getLine(), ctx.source.getText(), associationSymbol.getTarget()
                                                                                               .getName()));
        }
        ...
        if (resolvedSubMember instanceof FieldSymbol) {
            ...
            if (resolvedMemberType == null) {
                if (!nonResolvedRefSymbols.contains(resolvedSubMember)) {
                    ...
                    resolvedMemberType =
                            resolveReferenceChain(resolvedSubMemberField.getReference(), resolvedSubMemberField, nonResolvedRefSymbols);
    ...
    public Symbol resolveReference(String referencedId, Symbol referencingSymbol) {
        ...
        if (resolvedTypeSymbol == null && (referencingSymbol instanceof FieldSymbol || referencingSymbol instanceof AssociationSymbol)) {
            ...
            if (resolvedTypeSymbol == null) {
                throw new CDSRuntimeException(String.format("Error at line: %s. No such user defined type found: %s",
