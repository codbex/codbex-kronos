...
public class ProjectFilesModificator {
    ...
    private static final List<String> REPLACE_SESSION_USER_FILE_EXTENSIONS =
            List.of("xsjs", "xsjslib", HDB_PROCEDURE_FILE_EXTENSION, "hdbtablefunction", "analyticprivilege", "hdbanalyticprivilege");
    ...
    private void replaceSessionUser(String fileExtension, File projectFile) {
        if (REPLACE_SESSION_USER_FILE_EXTENSIONS.contains(fileExtension)) {
            ...
            projectFile.setContent(new String(currentContent).replace("SESSION_USER", "SESSION_CONTEXT('APPLICATIONUSER')")
                                                             .getBytes());
        }
    }
    ...
    private void modifyAnalyticPrivilegeFile(String fileExtension, File analyticPrivilegeFile) throws IOException {
        if (ANALYTIC_PRIVILEGE_FILE_EXTENSIONS.contains(fileExtension)) {
            ...
            if (currentContent.length != 0) {
                try {
                    String xslt = IOUtils.toString(ProjectFilesModificator.class.getClassLoader()
                                                                                .getResourceAsStream(XSLT_RESOURCE_PATH),
                    ...
                    logger.error("Analytic privilege" + analyticPrivilegeFile.getName() + "modification has failed due to an error.",
                            exception);
                    CommonsUtils.logCustomErrors(analyticPrivilegeFile.getPath(), CommonsConstants.MIGRATION_ERROR, "", "",
                            exception.getMessage(), "", CommonsConstants.HDB_ANALYTIC_PRIVILEGE, CommonsConstants.MODULE_PARSERS,
    ...
    /**
     * Modify analytic privilege file modelUri element by removing the reference of calculation view for
     * the / syntax.
     *
    ...
    private void replaceReservedWordRow(String fileExtension, File projectFile) {
        if (fileExtension.equalsIgnoreCase(HDB_PROCEDURE_FILE_EXTENSION)) {
            ...
            projectFile.setContent(new String(currentContent).replaceAll(ROW_DEFINITION, ROW_DEFINITION_REPLACEMENT)
                                                             .replaceAll(ROW_VALUE, ROW_VALUE_REPLACEMENT)
                                                             .getBytes());
        }
    }
    ...
    private void modifyUpdateFromStatement(String fileExtension, File projectFile) {
        if (fileExtension.equalsIgnoreCase(HDB_PROCEDURE_FILE_EXTENSION) && !projectFile.isEmpty()) {
            ...
            for (Map.Entry<String, String> mapping : modifiedUpdateStatementsMapping.entrySet()) {
                ...
                hdbprocedureFileContent =
                        hdbprocedureFileContent.replace(originalUpdateFromStatement + ";", modifiedUpdateFromStatement + ";");
            }
     ...
     * @return the string
     */
    private String modifyHdbprocedureUpdateFromWithoutJoinClauses(UpdateStatementDefinitionModel updateStatement,
        ...
        if (fromClauseTableReferences.size() > 2) {
            String errorMessage =
                    "In a HDBProcedures UPDATE FROM statement, we do not support automatic migration of more than two table references inside the FROM clause";
            logger.error(errorMessage);
            CommonsUtils.logCustomErrors("", CommonsConstants.MIGRATION_ERROR, "", "", errorMessage, "",
                    CommonsConstants.HDB_PROCEDURE_PARSER, CommonsConstants.MODULE_PARSERS, CommonsConstants.SOURCE_DELIVERY_UNIT_MIGRATION,
            ...
            mergeIntoWithoutJoinWith2FromStatements(modifiedUpdateStatement, whereClause, updatedTableReference, usingTableReference,
                    updateSetClauseRawContent);
        } else {
    ...
    private String modifyHdbprocedureUpdateFromWithJoinClauses(UpdateStatementDefinitionModel updateStatement,
            FromClauseDefinitionModel fromClause, List<JoinClauseDefinitionModel> joinClauses) {
        StringBuilder modifiedUpdateStatement = new StringBuilder();
        WhereClauseDefinitionModel whereClause = updateStatement.getWhereClause();
        TableReferenceModel fromClauseTableReference = fromClause.getTableReferences()
                                                                 .get(0);
        ...
        String updateSetClauseRawContent = updateStatement.getUpdateSetClause()
                                                          .getRawContent();
        ...
        modifiedUpdateStatement.append(whereClause != null ? AND + whereClause.getRawContent()
                                                                              .replaceFirst(CASE_INSENSITIVE_WHERE, "")
                + SPACE : "");
        modifiedUpdateStatement.append(THEN_UPDATE + NEW_LINE);
        modifiedUpdateStatement.append(TAB)
                               .append(updateSetClauseRawContent);
        return modifiedUpdateStatement.toString();
    }
     ...
     */
    private void mergeIntoWithUpdateTableName(StringBuilder modifiedUpdateStatement, TableReferenceModel fromClauseTableReference,
        ...
        modifiedUpdateStatement.append(fromClauseTableName)
                               .append(SPACE);
        modifiedUpdateStatement.append(addAsTableName(fromClauseTableAlias));
        modifiedUpdateStatement.append(TAB + USING);
        modifiedUpdateStatement.append(joinClauseForUpdate.getName())
                               .append(SPACE);
        modifiedUpdateStatement.append(addAsTableName(joinClauseForUpdate.getAlias()));
        modifiedUpdateStatement.append(TAB)
                               .append(getJoinClauseRawContent(joinClauses));
        modifiedUpdateStatement.append(TAB)
                               .append(joinClauseForUpdate.getOnPart())
                               .append(NEW_LINE);
    }
    ...
    private void mergeIntoWithoutJoinWith2FromStatements(StringBuilder modifiedUpdateStatement, WhereClauseDefinitionModel whereClause,
        ...
        modifiedUpdateStatement.append(MERGE_INTO)
                               .append(updatedTableReference.getName());
        modifiedUpdateStatement.append(addAsTableName(updatedTableReference.getAlias()));
        modifiedUpdateStatement.append(TAB + USING);
        modifiedUpdateStatement.append(usingTableReference.getName())
                               .append(SPACE);
    ...
    private String getWhereClause(WhereClauseDefinitionModel whereClause) {
        return whereClause != null ? ON + whereClause.getRawContent()
                                                     .replaceFirst(CASE_INSENSITIVE_WHERE, "")
                + SPACE : ON_TRUE;
    }
     ...
     */
    private TableReferenceModel getUpdatedTableReferenceModel(String updatedTableName,
            List<TableReferenceModel> fromClauseTableReferences) {
    ...
    private JoinClauseDefinitionModel getJoinClauseDefinitionModel(String updatedTableName, List<JoinClauseDefinitionModel> joinClauses) {
        ...
        for (JoinClauseDefinitionModel joinClauseDefinitionModel : joinClauses) {
            if (updatedTableName.equals(joinClauseDefinitionModel.getName())
                    || updatedTableName.equals(joinClauseDefinitionModel.getAlias())) {
    ...
    private String getJoinClauseRawContentWithoutTableName(String updatedTableName, List<JoinClauseDefinitionModel> joinClauses) {
        ...
        joinClauses.forEach(joinClauseDefinitionModel -> {
            if (!updatedTableName.equals(joinClauseDefinitionModel.getName())
                    && !updatedTableName.equals(joinClauseDefinitionModel.getAlias())) {
                updatedJoinClauseRawContent.append(TAB)
                                           .append(joinClauseDefinitionModel.getRawContent());
            }
        });
    ...
    private String getJoinClauseRawContent(List<JoinClauseDefinitionModel> joinClauses) {
        ...
        joinClauses.forEach(joinClause -> {
            joinClausesRawContent.append(TAB)
                                 .append(joinClause.getRawContent())
                                 .append(NEW_LINE);
        });
