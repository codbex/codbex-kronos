...
public class JobToKronosJobDefinitionTransformer {
    ...
    public ArrayList<XSJob> transform(JobArtifact jobArtifact) throws ParseException {
        ...
        String[] parseAction = jobArtifact.getAction()
                                          .split("::");
        ...
        if (parseAction.length == PARTS_OF_ACTION_PROPERTY) {
            ...
            for (int i = 0; i < jobArtifact.getSchedules()
                                           .size(); i++) {
                XSJob jobDefinition = new XSJob();
                String jobDefinitionName = jobArtifact.getAction() + "-" + i;
                String cronExpression = jobArtifact.getSchedules()
                                                   .get(i)
                                                   .getXscron();
                String quartzCronExpression = cronToQuartzCronTransformer.transform(cronExpression);

                jobDefinition.setName(jobDefinitionName);
                jobDefinition.setParametersAsMap(jobArtifact.getSchedules()
                                                            .get(i)
                                                            .getParameter());
                jobDefinition.setDescription(jobArtifact.getDescription() + " " + jobArtifact.getSchedules()
                                                                                             .get(i)
                                                                                             .getDescription());
            ...
            CommonsUtils.logProcessorErrors("Invalid xsjob artifact definition!", CommonsConstants.PROCESSOR_ERROR,
                    jobArtifact.getDescription(), CommonsConstants.JOB_PARSER);
