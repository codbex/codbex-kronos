...
public class HDBDDTransformer {
    ...
    public HDBTable transformEntitySymbolToTableModel(EntitySymbol entitySymbol, String location) {
        ...
        List<EntityElementSymbol> entityPks = entitySymbol.getElements()
                                                          .stream()
                                                          .filter(EntityElementSymbol::isKey)
                                                          .collect(Collectors.toList());
        HDBTableConstraintPrimaryKey primaryKey = new HDBTableConstraintPrimaryKey();
        primaryKey.setColumns(entityPks.stream()
                                       .map(EntityElementSymbol::getName)
                                       .toArray(String[]::new));
        ...
        entitySymbol.getAssociations()
                    .forEach(associationSymbol -> {
                        ...
                        String[] referencedColumns = associationColumns.stream()
                                                                       .map(HDBTableColumn::getName)
                                                                       .toArray(String[]::new);
                        ...
                        if (associationSymbol.isManaged()) {
                            ...
                            associationColumns.forEach(ac -> ac.setName(
                                    associationSymbolName.substring(associationSymbolName.indexOf(UNMANAGED_ASSOCIATION_MARKER) + 1)));
                            foreignKeyName = foreignKeyName.replace(UNMANAGED_ASSOCIATION_MARKER, "");
                        }

                        String[] foreignKeyColumns = associationColumns.stream()
                                                                       .map(HDBTableColumn::getName)
                                                                       .toArray(String[]::new);

                        foreignKeyModel.setName(foreignKeyName);
                        foreignKeyModel.setReferencedTable(associationSymbol.getTarget()
                                                                            .getFullName());
                        foreignKeyModel.setReferencedSchema(associationSymbol.getTarget()
                                                                             .getSchema());
                        ...
                        if (!associationSymbol.getForeignKeys()
                                              .isEmpty()) {
                            tableModel.getConstraints()
                                      .getForeignKeys()
                                      .add(foreignKeyModel);
        ...
        if (entitySymbol.getAnnotation(CATALOG_ANNOTATION) != null) {
            String tableType = entitySymbol.getAnnotation(CATALOG_ANNOTATION)
                                           .getKeyValuePairs()
                                           .get(CATALOG_OBJ_TABLE_TYPE)
                                           .getValue();
            tableModel.setTableType(tableType);

            if (entitySymbol.getAnnotation(CATALOG_ANNOTATION)
                            .getKeyValuePairs()
                            .get(INDEX) != null) {
                List<HDBTableIndex> indexes = new ArrayList<>();
                List<HDBTableConstraintUnique> uniqueIndexes = new ArrayList<>();
                AnnotationArray catalogIndexAnnotationArray = (AnnotationArray) entitySymbol.getAnnotation(CATALOG_ANNOTATION)
                                                                                            .getKeyValuePairs()
                                                                                            .get(INDEX);
                ...
                for (AbstractAnnotationValue currentAnnotationValue : catalogIndexAnnotationArray.getValues()) {
                    ...
                    ((AnnotationArray) annotationObject.getValue(ELEMENT_NAMES)).getValues()
                                                                                .forEach(currentElement -> indexColumnSet.add(
                                                                                        currentElement.getValue()));

                    if (!isUnique) {
                        indexes.add(new HDBTableIndex(name, "index", false, order, indexColumnSet.toArray(String[]::new), tableModel));
                    } else {
                        uniqueIndexes.add(new HDBTableConstraintUnique(name, null, indexColumnSet.toArray(String[]::new),
                                tableModel.getConstraints(), null, order));
                    }
                }
                tableModel.setIndexes(indexes);
                tableModel.getConstraints()
                          .setUniqueIndexes(uniqueIndexes);
            }
        }
    ...
    private void handlePossibleSearchIndexAnnotations(EntitySymbol entitySymbol, HDBTable tableModel) {
        for (int i = 0; i < entitySymbol.getElements()
                                        .size(); i++) {
            EntityElementSymbol currentElement = entitySymbol.getElements()
                                                             .get(i);

            if (currentElement.getAnnotation(SEARCH_INDEX_ANNOTATION) != null) {

                boolean hasFuzzySearchIndex = false;
                Map<String, AbstractAnnotationValue> searchIndexAnnotationValueMap = currentElement.getAnnotation(SEARCH_INDEX_ANNOTATION)
                                                                                                   .getKeyValuePairs();
                ...
                AbstractAnnotationValue fuzzyIndexAnnotationObjectValue = fuzzyIndexAnnotationObject != null
                        ? fuzzyIndexAnnotationObject.getKeyValuePairs()
                ...
                tableModel.getColumns()
                          .get(i)
                          .setFuzzySearchIndex(hasFuzzySearchIndex);
            }
        }
    }
    ...
    public HDBView transformViewSymbolToHdbViewModel(ViewSymbol viewSymbol, String location) {
        ...
        viewStatementSql.append(ISqlKeywords.KEYWORD_VIEW)
                        .append(SPACE)
                        .append(QUOTE)
                        .append(viewSymbol.getSchema())
                        .append(QUOTE)
                        .append(DOT)
                        .append(QUOTE)
                        .append(viewSymbol.getFullName())
                        .append(QUOTE)
                        .append(SPACE)
                        .append(ISqlKeywords.KEYWORD_AS)
                        .append(SPACE);
     ...
     * @return the string
     */
    public String traverseSelectStatements(ViewSymbol viewSymbol, List<String> aliasesForReplacement, HDBView viewModel) {
        ...
        for (Symbol symbol : viewSymbol.getSelectStatements()) {
            ...
            if (unionBol) {
                selectSql.append(ISqlKeywords.KEYWORD_UNION)
                         .append(SPACE);
            }

            selectSql.append(ISqlKeywords.KEYWORD_SELECT)
                     .append(SPACE);

            if (distinctBol) {
                selectSql.append(ISqlKeywords.KEYWORD_DISTINCT)
                         .append(SPACE);
            }

            selectSql.append(selectColumns)
                     .append(SPACE)
                     .append(ISqlKeywords.KEYWORD_FROM)
                     .append(SPACE)
                     .append(QUOTE)
                     .append(dependsOnTable)
                     .append(QUOTE)
                     .append(SPACE);

            if (dependingTableAlias != null) {
                selectSql.append(ISqlKeywords.KEYWORD_AS)
                         .append(SPACE)
                         .append(QUOTE)
                         .append(dependingTableAlias)
                         .append(QUOTE)
                         .append(SPACE);
            ...
            selectSql.append(joinStatements)
                     .append(SPACE);

            if (where != null) {
                selectSql.append(ISqlKeywords.KEYWORD_WHERE)
                         .append(SPACE)
                         .append(where)
                         .append(SPACE);
            }
    ...
    public String traverseJoinStatements(SelectSymbol selectSymbol, ViewSymbol viewSymbol, String dependsOnTable,
        ...
        for (Symbol symbol : selectSymbol.getJoinStatements()) {
            ...
            joinStatements.append(joinType)
                          .append(SPACE)
                          .append(QUOTE)
                          .append(joinArtifactName)
                          .append(QUOTE)
                          .append(SPACE);

            if (joinTableAlias != null) {
                joinStatements.append(ISqlKeywords.KEYWORD_AS)
                              .append(SPACE)
                              .append(QUOTE)
    ...
    public HDBTableType transformStructuredDataTypeToHdbTableType(StructuredDataTypeSymbol structuredDataTypeSymbol) {
        ...
        structuredDataTypeSymbol.getFields()
                                .forEach(field -> {
    ...
    private List<HDBTableColumn> transformAssociationToColumnModels(AssociationSymbol associationSymbol) {
        List<HDBTableColumn> tableColumns = new ArrayList<>();
        associationSymbol.getForeignKeys()
                         .forEach(fk -> {
    ...
    private void setSqlType(HDBColumn columnModel, BuiltInTypeSymbol builtInTypeSymbol) {
        ...
        if (cdsTypeEnum.equals(CdsTypeEnum.String)) {
            columnModel.setLength(builtInTypeSymbol.getArgsValues()
                                                   .get(0)
                                                   .toString());
        } else if (cdsTypeEnum.equals(CdsTypeEnum.Decimal)) {
            columnModel.setPrecision(builtInTypeSymbol.getArgsValues()
                                                      .get(0)
                                                      .toString());
            columnModel.setScale(builtInTypeSymbol.getArgsValues()
                                                  .get(1)
                                                  .toString());
        }
    ...
    private List<EntityElementSymbol> getStructuredTypeSubElements(FieldSymbol entityElementSymbol) {
        ...
        structuredDataType.getFields()
                          .forEach(field -> {
                              EntityElementSymbol subElement =
                                      new EntityElementSymbol(elementName + "." + field.getName(), entityElementSymbol.getScope());
     ...
     */
    private HDBTableColumn getAssociationForeignKeyColumn(AssociationSymbol associationSymbol, EntityElementSymbol foreignKey) {
    ...
    private String getCatalogAnnotationValue(AnnotationObj annotationObject, String value) {
        return annotationObject.getValue(value) != null ? annotationObject.getValue(value)
                                                                          .getValue()
                : null;
    }
}
