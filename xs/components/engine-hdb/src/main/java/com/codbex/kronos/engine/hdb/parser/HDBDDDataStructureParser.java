...
public class HDBDDDataStructureParser implements HDBDataStructureParser<HDBDD> {
    ...
    private void parseHDBDD(String location, String content) throws IOException, ArtifactParserException {
        ...
        CommonsUtils.logParserErrors(lexerErrorListener.getErrors(), CommonsConstants.LEXER_ERROR, location, CommonsConstants.HDBDD_PARSER);
        ...
        try {
            ...
            CommonsUtils.logCustomErrors(location, CommonsConstants.PARSER_ERROR, "", "", e.getMessage(), "", CommonsConstants.HDBDD_PARSER,
                    CommonsConstants.MODULE_PARSERS, CommonsConstants.SOURCE_PUBLISH_REQUEST, CommonsConstants.PROGRAM_KRONOS);
            throw new CDSRuntimeException(String.format("Failed to parse file: %s. %s", location, e.getMessage()));
        }

        artifactDefinitionListener.getPackagesUsed()
                                  .forEach(p -> {
    ...
    private void getRootFiles(String usedFileName, List<String> rootFiles) {
        ...
        dependencyStructure.get(usedFileName)
                           .forEach(f -> {
    ...
    private void getCdsModelWithParsedData(HDBDD cdsModel) {
        ...
        parsedStructuredDataTypes.forEach(sdt -> {
            if (!(sdt.getAnnotations()
                     .containsKey("GenerateTableType"))
                    || (sdt.getAnnotation("GenerateTableType")
                           .getKeyValuePairs()
                           .get("booleanValue")
        ...
        parsedViews.forEach(v -> viewModels.add(this.hdbddTransformer.transformViewSymbolToHdbViewModel(v, cdsModel.getLocation())));
