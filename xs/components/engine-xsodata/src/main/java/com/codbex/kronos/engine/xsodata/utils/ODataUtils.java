...
public class ODataUtils {
    ...
    public OData convertODataModelToODataDefinition(XSOData oDataModel) {
        ...
        String namespace = oDataModel.getService()
                                     .getNamespace() != null ? oDataModel.getService()
                                                                         .getNamespace()
                                             : "Default";
        ...
        for (XSODataEntity entity : oDataModel.getService()
            ...
            entity.getNavigates()
                  .forEach(processNavigation(oDataModel, oDataDefinitionModel, oDataEntityDefinition));
            ...
            try {
                ...
                if (ISqlKeywords.METADATA_CALC_VIEW.equals(tableMetadata.getKind()) && entity.getWithPropertyProjections()
                                                                                             .isEmpty()
                        && entity.getWithoutPropertyProjections()
                                 .isEmpty()) {
                    ...
                    allEntityDbColumns.forEach(el -> {
                        ...
                        oDataEntityDefinition.getProperties()
                                             .add(oDataProperty);
                    });
                }

                entity.getWithPropertyProjections()
                      .forEach(prop -> {
                          ODataProperty oDataProperty = new ODataProperty();
                          oDataProperty.setName(prop);
                          oDataProperty.setColumn(prop);
                          List<TableColumn> dbProp = allEntityDbColumns.stream()
                                                                       .filter(x -> x.getName()
                                                                                     .equals(prop))
                                                                       .collect(Collectors.toList());
                          if (!dbProp.isEmpty()) {
                              oDataProperty.setNullable(dbProp.get(0)
                                                              .isNullable());
                              oDataProperty.setType(dbProp.get(0)
                                                          .getType());
                          }
                          oDataEntityDefinition.getProperties()
                                               .add(oDataProperty);
                ...
                if (!entity.getWithoutPropertyProjections()
                           .isEmpty()) {
                    allEntityDbColumns.forEach(el -> {
                        if (entity.getWithoutPropertyProjections()
                                  .stream()
                                  .noneMatch(x -> x.equals(el.getName()))) {
                            ODataProperty oDataProperty = new ODataProperty();
                            oDataProperty.setName(el.getName());
                            oDataProperty.setColumn(el.getName());
                            oDataProperty.setNullable(el.isNullable());
            ...
            entity.getModifications()
                  .forEach(processModification(oDataEntityDefinition, handlers));
            handlers.forEach(el -> oDataEntityDefinition.getHandlers()
                                                        .add(el));
     ...
     * @return the consumer
     */
    private Consumer<XSODataModification> processModification(ODataEntity oDataEntityDefinition, List<ODataHandler> handlers) {
        return modification -> {
            modification.getSpecification()
                        .getEvents()
                        .forEach(event -> {
                            if (validateHandlerType(event.getType())) {
                                ODataHandler oDataHandler = new ODataHandler();
                                oDataHandler.setHandler(event.getAction());
                                oDataHandler.setType(event.getType()
                                                          .getOdataHandlerType());
                                oDataHandler.setMethod(modification.getMethod()
                                                                   .getOdataHandlerType());
                                handlers.add(oDataHandler);
                            }
                        });
            if (modification.getSpecification()
                            .isForbidden()) {
                ODataHandler oDataHandler = new ODataHandler();
                ...
                oDataHandler.setMethod(modification.getMethod()
                                                   .getOdataHandlerType());
                handlers.add(oDataHandler);
                oDataEntityDefinition.getAnnotationsEntitySet()
                                     .put(modification.getMethod()
                                                      .getOdataSAPAnnotation(),
                                             "false");
            }
            if (modification.getSpecification()
                            .getModificationAction() != null) {
                ODataHandler oDataHandler = new ODataHandler();
                oDataHandler.setHandler(modification.getSpecification()
     ...
     * @return the consumer
     */
    Consumer<XSODataNavigation> processNavigation(XSOData oDataModel, OData oDataDefinitionModel, ODataEntity oDataEntityDefinition) {
        return navigate -> {
            ...
            XSODataAssociation xsOdataAssoc = getAssociation(oDataModel, navigate.getAssociation(), navigate.getAliasNavigation());

            ODataAssociationEnd fromDef = new ODataAssociationEnd();
            fromDef.setEntity(xsOdataAssoc.getPrincipal()
                                          .getEntitySetName());

            // The Multiplicity of the Principal role must be 1 or 0..1
            validateEdmMultiplicity(xsOdataAssoc.getPrincipal()
                                                .getMultiplicityType()
                                                .getText(),
                    navigate.getAssociation());
            fromDef.setMultiplicity(xsOdataAssoc.getPrincipal()
                                                .getMultiplicityType()
                                                .getText());
            fromDef.setProperties(xsOdataAssoc.getPrincipal()
                                              .getBindingRole()
                                              .getKeys());
            ODataAssociationEnd toDef = new ODataAssociationEnd();
            toDef.setEntity(xsOdataAssoc.getDependent()
                                        .getEntitySetName());

            // The Multiplicity of the Principal role must be 1, 0..1, 1..*, *
            // convert 1..* to *, because odata do not support it
            if (xsOdataAssoc.getDependent()
                            .getMultiplicityType()
                            .getText()
                            .equals(XSODataMultiplicityType.ONE_TO_MANY.getText())) {
                toDef.setMultiplicity(EdmMultiplicity.MANY.toString());
            } else {
                validateEdmMultiplicity(xsOdataAssoc.getDependent()
                                                    .getMultiplicityType()
                                                    .getText(),
                        navigate.getAssociation());
                toDef.setMultiplicity(xsOdataAssoc.getDependent()
                                                  .getMultiplicityType()
                                                  .getText());
            }

            toDef.setProperties(xsOdataAssoc.getDependent()
                                            .getBindingRole()
                                            .getKeys());
            ...
            if (xsOdataAssoc.getDependent()
                            .getMultiplicityType()
                            .getText()
                            .equals(EdmMultiplicity.MANY.toString())
                    && xsOdataAssoc.getPrincipal()
                                   .getMultiplicityType()
                                   .getText()
                                   .equals(EdmMultiplicity.MANY.toString())) {

                fromDef.getMappingTable()
                       .setMappingTableName(xsOdataAssoc.getAssociationTable()
                                                        .getRepositoryObject());
                fromDef.getMappingTable()
                       .setMappingTableJoinColumn(xsOdataAssoc.getAssociationTable()
                                                              .getPrincipal()
                                                              .getKeys()
                                                              .get(0));

                toDef.getMappingTable()
                     .setMappingTableName(xsOdataAssoc.getAssociationTable()
                                                      .getRepositoryObject());
                toDef.getMappingTable()
                     .setMappingTableJoinColumn(xsOdataAssoc.getAssociationTable()
            ...
            oDataDefinitionModel.getAssociations()
                                .add(oDataAssociationDefinition);
        };
    }
    ...
    /**
     * Validate if provided handler type is one of the
    ...
    private void getParametersForCalcView(List<PersistenceTableColumnModel> allEntityParameters, String tableName) throws SQLException {
        try (Connection connection = getDataSource().getConnection()) {
            try (PreparedStatement statement =
                    connection.prepareStatement("SELECT DISTINCT VARIABLE_NAME, COLUMN_SQL_TYPE, MANDATORY, \"ORDER\" "
     ...
     * @param tableName the table name
     */
    private void processParameters(OData oDataDefinitionModel, ODataEntity oDataEntityDefinition, XSODataEntity entity,
            List<PersistenceTableColumnModel> allEntityParameters, String tableName) {
        ODataEntity oDataEntityParametersDefinition = new ODataEntity();

        String parameterEntitySetName = entity.getParameterEntitySet()
        ...
        allEntityParameters.forEach(el -> {
            ...
            oDataEntityDefinition.getParameters()
                                 .add(oDataParameter);
            oDataEntityParametersDefinition.getParameters()
                                           .add(oDataParameter);
        });
        ...
        oDataDefinitionModel.getAssociations()
                            .add(oDataParametersAssociation);

        ODataNavigation oDataResultsNavigation = new ODataNavigation();
        oDataResultsNavigation.setName(entity.getParameterEntitySet()
                                             .getParameterResultsProperty());
        oDataResultsNavigation.setAssociation(oDataParametersAssociation.getName());
        oDataEntityParametersDefinition.getNavigations()
                                       .add(oDataResultsNavigation);

        oDataDefinitionModel.getEntities()
                            .add(oDataEntityParametersDefinition);
    }
    ...
    public static XSODataAssociation getAssociation(XSOData model, String name, String navigation) {
        if (model != null && model.getService() != null) {
            for (XSODataAssociation association : model.getService()
                                                       .getAssociations()) {
